...
| M21-019 | Single App and service per data directory; never kill a compatible external service  | native lifecycle tests, runtime-lock integration, manual conflict replay         | Required                            |
| M21-020 | `Cmd-W` retains state; Dock activation reopens in ≤ 300 ms p95                       | native test and packaged samples                                                 | Required                            |
| M21-021 | `Cmd-Q` reveals hidden confirmation, bounds flush/shutdown, and never silently kills | bridge/native tests, user-wait/10-second timeout tests, 30-cycle replay          | Required                            |
